...
class _ViperMenuCentralState extends State<ViperMenuCentral> {
        ...
        children: [
          Container(
            ...
            child: CircleAvatar(
              ...
              child: (_menuController.driverProfile?['avatar_url'] != null && _menuController.driverProfile?['avatar_url'].toString().isNotEmpty == true)
                  ? ClipOval(
                      child: CachedNetworkImage(
                        imageUrl: _menuController.driverProfile!['avatar_url'],
  ...
  Widget _buildVehicleView(bool isDark, Color textColor) {
    ...
    return _buildPandaContainer(
      ...
      Column(
        children: [
          _buildInfoItem(Icons.local_shipping_outlined, 'MODELO', model, textColor, isDark),
          const SizedBox(height: 12),
          Divider(color: isDark ? Colors.white12 : Colors.black12),
          const SizedBox(height: 12),
          _buildInfoItem(Icons.palette_outlined, 'COR', color.toUpperCase(), textColor, isDark),
          const SizedBox(height: 12),
          Divider(color: isDark ? Colors.white12 : Colors.black12),
          const SizedBox(height: 12),
          _buildInfoItem(Icons.tag, 'PLACA', plate, textColor, isDark, highlight: true),
        ],
      )
    );
  }
  ...
  Widget _buildProfileView(bool isDark, Color textColor) {
    final photoUrl = _menuController.driverProfile?['avatar_url'] as String?;
    ...
    return _buildPandaContainer(
      ...
      Column(
        children: [
            ...
            children: [
                ...
                child: ClipOval(
                  child: (photoUrl != null && photoUrl.isNotEmpty) 
                    ? CachedNetworkImage(
                        ...
                        errorWidget: (context, url, error) => Icon(Icons.person, size: 20, color: textColor.withValues(alpha: 0.5)),
                      )
                    : Icon(Icons.person, size: 20, color: textColor.withValues(alpha: 0.5)),
                ),
              ),
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('NOME COMPLETO', style: TextStyle(color: textColor.withValues(alpha: 0.4), fontSize: 8, fontWeight: FontWeight.bold)),
                    Text('$firstName $lastName'.toUpperCase(), style: TextStyle(color: textColor, fontSize: 13, fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
            ],
          ...
          Divider(color: isDark ? Colors.white12 : Colors.black12),
          const SizedBox(height: 12),
          _buildInfoItem(Icons.home_outlined, 'BASE OPERACIONAL', '$city - $state'.toUpperCase(), textColor, isDark),
        ],
      )
    );
  }
  ...
  Widget _buildCNHView(bool isDark, Color textColor) {
    ...
    final expiration = _menuController.driverProfile?['cnh_expiration'] ?? '01/01/2030';
    final verified = _menuController.driverProfile?['cnh_verified'] ?? true; // Default mock
      ...
      Column(
        children: [
          Row(
            ...
            children: [
              _buildInfoItem(Icons.credit_card_outlined, 'CATEGORIA', cat, textColor, isDark),
              if (verified)
                Container(
                  ...
                  child: const Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      Icon(Icons.check_circle, color: Color(0xFF00FF88), size: 12),
                      SizedBox(width: 4),
                      Text('VERIFICADO', style: TextStyle(color: Color(0xFF00FF88), fontSize: 9, fontWeight: FontWeight.bold)),
                    ],
                  ),
                ),
            ],
          ...
          Divider(color: isDark ? Colors.white12 : Colors.black12),
          const SizedBox(height: 12),
          _buildInfoItem(Icons.calendar_today_outlined, 'VALIDADE', expiration, textColor, isDark),
          const SizedBox(height: 12),
          Text(
            'Documento digital sincronizado com a base Viper.',
            style: TextStyle(color: textColor.withValues(alpha: 0.3), fontSize: 9, fontStyle: FontStyle.italic),
          ),
        ],
      )
    );
  ...
  Widget _buildSelectedPerformanceView(bool isDark, Color textColor) {
    switch (_selectedView) {
      ...
      case PerformanceView.week:
        final totalWeekly = _menuController.weeklyEarnings.reduce((a, b) => a + b);
        return Column(
          children: [
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
  ...
  Widget _buildDayView(bool isDark, Color textColor) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: isDark ? Colors.white12 : Colors.black, width: 2.0),
      ),
      child: Column(
        children: [
          ...
          const SizedBox(height: 16),
          _buildMiniStat('Entregas Realizadas', '${_menuController.dailyDeliveries}', isDark, textColor),
        ],
      ),
    );
  }
  ...
  Widget _buildMonthView(bool isDark, Color textColor) {
    return Container(
      ...
      child: Column(
        children: [
          Text('FATURAMENTO DO MÊS', style: TextStyle(color: textColor.withValues(alpha: 0.3), fontSize: 10, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          Text(
            'R\$ ${_menuController.monthlyEarnings.toStringAsFixed(2)}',
            style: TextStyle(color: isDark ? const Color(0xFF00FF88) : const Color(0xFF0055FF), fontSize: 26, fontWeight: FontWeight.w900),
          ),
          const SizedBox(height: 20),
          Row(
            children: [
              Expanded(child: _buildMiniStat('Volume Mensal', '${_menuController.monthlyDeliveries} envios', isDark, textColor)),
              const SizedBox(width: 12),
              Expanded(child: _buildMiniStat('Média por Pedido', 'R\$ ${(_menuController.monthlyDeliveries > 0 ? _menuController.monthlyEarnings / _menuController.monthlyDeliveries : 0).toStringAsFixed(2)}', isDark, textColor)),
            ],
          ),
        ],
      ),
